...
class Graph:
    ...
    def get_graph_from_file(path: str):
        with open(path, 'r') as f:
            ...
            for source, dest, order in graph_edges:
                d = dest
                if not d in dict_input_vertex:
                    if order == 1:
                        dict_input_vertex[d] = 1
                        #print(dict_input_vertex[])
                    else:
                        sys.exit('Ошибка в входном файле')
                else:
                    if order == dict_input_vertex[d] + 1:
                        dict_input_vertex[d] += 1
                    else:
                        sys.exit('Ошибка в входном файле')
    ...
    def open_operation(self):
        with open('operations.txt', 'r') as f:
            operation_text = f.read()
            text = re.findall(r'\S : [*+\d]', operation_text)
            text_exp = re.findall(r'\S : exp', operation_text)
            text = text + text_exp
            vert_dict = {}
            for i in text:
                txt = i.split(' : ')
                vert_dict[txt[0]] = txt[1]
        return vert_dict
    ...
    def three_exercise(self, vert_dict):
...
def main():
    ...
    rg.two_exercise()
    vert_dict = rg.open_operation()
    rg.three_exercise(vert_dict)
