...
def hypotheses(n1, n2):
    ...
    encrypted_text = open('encrypted_text.txt', 'r', encoding='UTF-8').read().lower()
    P0 = []
    with open('h0.txt', 'r') as hypot_in:
    ...
    with open('values.txt', 'w') as values_out:
        for value in sorted_values:
            values_out.write("{}\t\t{:.5f}\n".format(value[0], value[1]))
    with open('hypothesis.txt', 'w') as hypot_out:
...
def main():
    while True:
        cmd = int(input("1 Шифр Виженера\n"
                        ...
                        "3 Частотный анализ и составление гиптотезы H(0)\n"
                        "4 Гипотезы\n\nВведите команду: "))
        if cmd == 1:
            vigenere_encrypt()
        elif cmd == 2:
            ...
            while n2 <= n1:
                n2 = int(input('Введено неправильно значение, введите другое n2: '))
            hypotheses(n1, n2)
