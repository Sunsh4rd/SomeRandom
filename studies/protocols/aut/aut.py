...
def print_trees(list_of_matrix):
    ...
    for i in list_of_matrix:
        if list_of_matrix.index(i) != len(list_of_matrix[0][0][1]):
            string += 'Уровень: ' + str(list_of_matrix.index(i)) + '\n'
...
def write_weight_in_file(maximum):
    f = open('width.txt', 'w', encoding='utf-8')
    f.write('Упорядоченное множество имеет ширину ' + str(maximum))
    f.close()
...
programm()
print("Дерево матриц построено, ширина у.м. найдена")
